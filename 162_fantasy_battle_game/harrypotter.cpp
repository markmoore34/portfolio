/*************************************************************
 Name: Mark Moore
 Date: 10.30.17
 Description: This program is a fantasy combat game. The user will choose two characters the the characters will battle to the death.
 ***************************************************************/

#include "harrypotter.hpp"


Harrypotter::Harrypotter(std::string nam, int armr, int strength, int lives)
{
	name = nam;
	armor = armr;
	strengthPts = strength;
	numLives = lives;
	//std::cout << "\nTEST Harry Potter created.";
	//std::cout << name << "...." << armor << "...." << strengthPts << "...." << numLives << std::endl << std::endl;
}

int Harrypotter::getDamage(int oRoll, int dRoll)// gets roll numbers then adjusts variables accordingly.
{
	int damage = (oRoll - dRoll - armor);
	if(damage > 0)
		strengthPts = strengthPts - damage;
	
	if(strengthPts <= 0)
	{
		strengthPts = 20;
		return damage;
	}
	else
	{
		damage = 0;
		return damage;
	}
}
