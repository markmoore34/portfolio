...
Bennigans::Bennigans(std::string p, std::string place)
{
	prize = p;
	this->place = place;
}

std::string Bennigans::riddle()//will have the riddle and answer choices. will return the string for the answer.
{
	printMap();
	std::cout << "You enter to find a note on the floor. \nIt reads, 'Why do French people like to eat snails?'. \nYou need to choose A, B, or C, to answer." << std::endl;
	
	char choice;
	std::cout << "\n\tA : They don't like eating fast food.\n";//correct
	std::cout << "\tB : Because the frog legs jump away.\n";
	std::cout << "\tC : So they don't starve.\n";
